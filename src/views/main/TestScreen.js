...
// ─── RenderItemCollapsible is defined OUTSIDE TestScreen ─────────────────────
// This is the critical fix: defining it inside TestScreen causes its component
// type to be recreated on every parent render, making React unmount/remount it
// and wiping all local state (including notes mid-typing).
const RenderItemCollapsible = memo(forwardRef(({
  item,
  index,
  isExpanded,
  onToggle,
  // All values previously closed over from TestScreen scope, now passed as props
  routeParams,
  localItems,
  currentUser,
  theme,
  ...
  const updateDressSubTypePart = useCallback((part, value) => {
    setLocalState(prev => {
      ...
      return { ...prev, dressSubType: newValue };
    });
  }, []);
  ...
  const handleMeasurementEdit = useCallback(() => {
    ...
    let itemFin = JSON.parse(JSON.stringify({ ...item, ...localState }));
    ...
    queueMicrotask(() => {
      if (isChecked) {
        ...
        if (firstItem) {
          ...
          setLocalState(prev => ({ ...prev, ...designProperties }));
        }
      } else {
        ...
        setLocalState(prev => ({ ...prev, ...resetProperties }));
      }
    });
    ...
    queueMicrotask(() => {
      if (isChecked) {
        ...
        if (firstItem) {
          ...
          setLocalState(prev => ({ ...prev, ...measurementProperties }));
        } else {
  ...
  const handleDeleteImage = useCallback((imageIndex, imageArray, imageType) => {
    ...
    if (imageType === 'measurementPics') {
      setSelectedItem(prev => ({ ...prev, measurementPics: newImages }));
    }
  ...
  const updateMeasurement = (index, field, newValue) => {
    setSelectedItem((prevSelectedItem) => {
      ...
      return { ...prevSelectedItem, extraMeasurements: updatedExtraMeasurements };
    });
  };
                ...
                {selectedItemDesign.sleeveType === 'Custom' && (
                  selectedItemDesign.sleeveDesignFile ? (
                    <View style={{ marginHorizontal: 40 }}>
                      ...
                      <Button style={styles.editButton} size='tiny' onPress={editDesign}>
                        Edit
                      </Button>
                    </View>
                  ) : (
                    <Button style={styles.drawButton} size='small' onPress={editDesign}>
                      Draw sleeve design
                    </Button>
                  )
                )}
        ...
        <Layout style={styles.tabContent}>
          ...
          >
            ...
            <View style={{ marginVertical: 12, alignItems: 'center' }}>
              <View style={{ position: 'absolute', width: '100%', height: 1, backgroundColor: '#E4E9F2', top: '50%' }} />
              <Text category='c1' style={{ backgroundColor: '#FFFFFF', paddingHorizontal: 12, color: '#8F9BB3', fontWeight: '500' }}>
                OR
              </Text>
            </View>
            ...
            <View style={{ marginVertical: 16 }}>
              ...
              {selectedItem.measurementPics?.length > 0 ? (
                  ...
                  {selectedItem.measurementPics.map((image, index) => (
                    <View key={index} style={styles.imageContainerMeas}>
                      ...
                      <Button
                        ...
                        style={{ position: 'absolute', top: -5, right: -5, width: 25, height: 25, borderRadius: 12.5, backgroundColor: 'rgba(255,0,0,0.8)' }}
                ...
                <Button
                  ...
                  style={{ marginBottom: 15, opacity: 1 }}
                >
            ...
            <TouchableOpacity
              style={[styles.collapseHeader, { paddingVertical: 12, paddingHorizontal: 4, marginHorizontal: -10, marginBottom: 8 }]}
                ...
                {fields.map((field) => (
                  <View key={field.key} style={styles.fieldContainer2}>
                    ...
                    <Input
                      ...
                      size='small'
                      onChangeText={(value) => {
                        const updatedMeasurementData = { ...selectedItem.measurementData, [field.key]: value };
                        const updItem = { ...selectedItem, measurementData: updatedMeasurementData };
          ...
          <View style={styles.buttonGroup}>
            <Button size='small' onPress={saveAllMeas}>Save</Button>
            <Button size='small' appearance='outline' onPress={() => setShowMeas(false)}>Cancel</Button>
          </View>
        </Layout>
      </Modal>
    )
  ...
  const renderExpandedContent = () => {
    ...
    return (
      <Layout style={styles.collapsibleContent}>
              ...
              <Select
                ...
                onSelect={(index) => {
                  ...
                  routeParams?.itemName === 'salwar'
                    ? updateDressSubTypePart('top', optionsSub[index.row])
                    : updateLocalState('dressSubType', optionsSub[index.row]);
                }}
              >
        ...
        {currentUser.userType !== 'production' && (
          <Layout style={styles.dateContainer}>
            ...
            <Button size='small' onPress={navigateToSlotScreen}>
              {Object.keys(item.slots).length > 0 ? 'Edit slots' : 'Select slots'}
            </Button>
          </Layout>
        )}
        ...
        {(index === 0 || !localState.repeatDesign) && showSubDropdown && (
          <Layout style={styles.priceContainer}>
            <Layout style={styles.innerLayout}>
              ...
              <Text category="s1" style={{ marginLeft: 4 }}>Neck & Sleeve</Text>
            </Layout>
            <Button
              appearance="ghost"
              status="primary"
          ...
          <ScrollView contentContainerStyle={{ flexGrow: 1 }} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={true}>
            <Layout style={styles.priceContainer}>
              <Layout style={styles.innerLayout}>
                ...
                <Text category="s1" style={{ marginLeft: 4 }}>Measurements *</Text>
              </Layout>
              <Button
                appearance="ghost"
                status="primary"
      ...
      {!isExpanded && (
        <Layout style={{ marginHorizontal: 10, marginVertical: 10 }}>
          <Text category='s2' appearance='hint'>
            {[
              localState.dressSubType,
              localState.frontNeckType ? `Front Neck: ${localState.frontNeckType}` : null,
              localState.backNeckType ? `Back Neck: ${localState.backNeckType}` : null,
              localState.sleeveType ? `Sleeve: ${localState.sleeveType}` : null,
...
const TestScreen = ({ route }) => {
  ...
  const awarenessOptions = [
    'Google Search', 'Facebook', 'Instagram', 'WhatsApp',
    'Friend/Family Referral', 'Walked by the store', 'Previous Customer', 'Other'
  ];
  ...
  const saveAllLocalStates = useCallback((index, newState) => {
  ...
  const getMeasurementFields = useCallback((dressType) => {
  ...
  const updateItemMultiple = useCallback((itemId, changes) => {
    let updatedItems;
    setLocalItems(prevItems => {
      updatedItems = prevItems.map(item =>
        item.id === itemId
  ...
  const deleteItemAlert = useCallback((id, index) => {
    Alert.alert(
      "Confirmation", "Do you want to delete this item?",
      [
        { text: 'Cancel', onPress: () => console.log("Cancel"), style: "cancel" },
        { text: 'OK', onPress: () => deleteItem(id, index) }
      ],
      { cancelable: true }
    )
  }, []);
  ...
  const onChangeText = async (query) => {
    ...
    if (query === '') {
      ...
      const filteredData = custList.filter((item) => item.phoneNo.includes(query));
  ...
  useEffect(() => {
    const handleAppGoingToBackground = async () => {
      ...
      setNewOrderCust({ custName, phoneNo: phNo, occasion });
    };
  }, []);
  ...
  const checkUpcomingDates = (anniversary, dob) => {
    ...
    if (anniversary) {
      ...
      if (thisYearAnniversary < today) { targetAnniversary = nextYearAnniversary; }
      ...
      if (daysUntilAnniversary >= 0 && daysUntilAnniversary <= 45) {
        upcomingEvents.push({ type: 'Anniversary', date: targetAnniversary, days: daysUntilAnniversary });
      }
    }
    ...
    if (dob) {
      ...
      if (thisYearBirthday < today) { targetBirthday = nextYearBirthday; }
      const daysUntilBirthday = Math.ceil((targetBirthday - today) / (1000 * 60 * 60 * 24));
      if (daysUntilBirthday >= 0 && daysUntilBirthday <= 45) {
        upcomingEvents.push({ type: 'Birthday', date: targetBirthday, days: daysUntilBirthday });
      }
    }
  ...
  const PersonIconLocal = (props) => <Icon {...props} name="person-outline" />;
  const CalendarIcon = (props) => <Icon {...props} name="calendar-outline" />;
  const PinIcon = (props) => <Icon {...props} name="pin-outline" />;
  const BriefcaseIcon = (props) => <Icon {...props} name="briefcase-outline" />;
  ...
  const saveCustDetails = async () => {
    ...
    if (existingCustomer) {
      setNewOrderCust({ custName: custName, phoneNo: phNo, occasion: occasion, custInserted: false });
      ...
      setNewOrderCust({ custName: custName, phoneNo: phNo, occasion: occasion, custInserted: true });
    ...
    return (
      <Card style={styles.newCustomerCard}>
        <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginTop: -5 }}>
          ...
          {customerData && <Button size='medium' appearance='ghost' accessoryLeft={CloseIcon} onPress={() => { setCustomerDataFound(true); }} />}
        </View>
    ...
    if (upcomingEvents.length > 0) {
      return (
        <Card style={styles.celebrationCard}>
          <Text category="h6" style={styles.celebrationTitle}>🎉 Upcoming Celebrations!</Text>
            ...
            <View key={index} style={styles.celebrationItem}>
              ...
              event.days === 0 ? styles.todayBadge :
                ...
                event.days === 0 ? styles.todayText :
                  ...
                  {event.days === 0 ? 'Today!' : event.days === 1 ? 'Tomorrow' : `${event.days} days`}
                </Text>
              </View>
            </View>
          ))}
      ...
      return (
        <Card style={styles.celebrationCard}>
          <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginTop: -5 }}>
            <Text category="h6">Customer Info</Text>
            <Button size='medium' appearance='ghost' accessoryLeft={EditIcon} onPress={handleEditFields} />
          </View>
          <View style={styles.celebrationRow}>
            <View style={[styles.celebrationItem, styles.celebrationItemHalf]}>
              <View style={styles.celebrationLeft}>
                <Text category="s1" style={styles.celebrationType}>🎂 Date of Birth</Text>
                <Text category="p2" style={styles.celebrationVal}>{customerData.dob}</Text>
              </View>
            </View>
            <View style={[styles.celebrationItem, styles.celebrationItemHalf]}>
              <View style={styles.celebrationLeft}>
                <Text category="s1" style={styles.celebrationType}>💕 Anniversary</Text>
                <Text category="p2" style={styles.celebrationVal}>{customerData.anniversary}</Text>
              </View>
            </View>
          </View>
          <View style={styles.celebrationRow}>
            <View style={[styles.celebrationItem, styles.celebrationItemHalf]}>
              <View style={styles.celebrationLeft}>
                <Text category="s1" style={styles.celebrationType}>📍Location</Text>
                <Text category="p2" style={styles.celebrationVal}>{customerData.location}</Text>
              </View>
            </View>
            <View style={[styles.celebrationItem, styles.celebrationItemHalf]}>
              <View style={styles.celebrationLeft}>
                <Text category="s1" style={styles.celebrationType}>💼 Profession</Text>
                <Text category="p2" style={styles.celebrationVal}>{customerData.profession}</Text>
              </View>
            </View>
          </View>
          <View style={styles.celebrationRow}>
            <View style={[styles.celebrationItem, styles.celebrationItemHalf]}>
              <View style={styles.celebrationLeft}>
                <Text category="s1" style={styles.celebrationType}>📣 Heard Via</Text>
                <Text category="p2" style={styles.celebrationVal}>{customerData.awareness_source}</Text>
              </View>
            </View>
          </View>
        </Card>
  ...
  const addItem = async () => {
    ...
    if (localItems.length > 0) {
      ...
      const newItem = {
        id: uniqId, localId: newCo, dressType: routeParams?.itemName,
        dressSubType: firstItem.dressSubType, dueDate: firstItem.dueDate,
        stitchingAmt: firstItem.stitchingAmt, frontNeckType: firstItem.frontNeckType,
        backNeckType: firstItem.backNeckType, sleeveType: firstItem.sleeveType,
        sleeveLength: firstItem.sleeveLength, measurementData: measurementData,
        notes: '', frontNeckDesignFile: firstItem.frontNeckDesignFile,
        backNeckDesignFile: firstItem.backNeckDesignFile, sleeveDesignFile: firstItem.sleeveDesignFile,
        dressPics: [], patternPics: [], measurementPics: [],
        dressGiven: firstItem.dressGiven, alterDressType: "",
        extraMeasurements: firstItem.extraMeasurements, editable: true,
        repeatDesign: true, repeatMeas: true, extraOptions: {},
        slotDates: [], slots: {}, savedDesign: false, savedMeas: false
      };
      console.log("newItem if: ", newItem);
      setCount(newCo);
      setLocalItems(prev => {
        const updated = [...prev, newItem];
        setExpandedItems([...Array(updated.length - 1).fill(false), true]);
        return updated;
      });
    } else {
      const newCo = count + 1;
      ...
      const measurementData = {};
      const fields = getMeasurementFields(routeParams?.itemName);
      fields.forEach(field => { measurementData[field.key] = ''; });
      const newItem = {
        id: uniqId, localId: newCo, dressType: routeParams?.itemName,
        dressSubType: optionsSub[0], dueDate: new Date(), stitchingAmt: '',
        frontNeckType: "", backNeckType: "", sleeveType: "", sleeveLength: "",
        measurementData: measurementData, notes: '', frontNeckDesignFile: null,
        backNeckDesignFile: null, sleeveDesignFile: null,
        dressPics: [], patternPics: [], measurementPics: [],
        dressGiven: false, alterDressType: "", extraMeasurements: {},
        editable: true, repeatDesign: false, repeatMeas: false, extraOptions: {},
        slotDates: [], slots: {}, savedDesign: false, savedMeas: false
      };
  ...
  const deleteItem = (id, indexToRemove) => {
    ...
    setExpandedItems(prev => {
      if (indexToRemove === prev.length - 1) { return prev.slice(0, -1); }
  ...
  const handleSave = async () => {
    ...
    if (!allItemsSaved) { allItemsSaved = localItems; }
    console.log(custName + ',' + phoneNo)
    try {
      setLoading(true);
      console.log('in save order else')
      const phNo = phoneNo.includes('+91') ? phoneNo : '+91' + phoneNo
      const updatedItems = allItemsSaved?.map(item => ({ ...item }));
  ...
  const resetAllMeasurements = (newMeasurementData) => {
    setLocalItems(prevItems =>
      prevItems.map(item => ({ ...item, measurementData: { ...newMeasurementData } }))
    );
  };
  ...
  const checkIfCustExists = async (resetExisting) => {
    ...
    if (orderFor !== 'Other') {
      ...
      if (measure) {
        ...
        if (resetExisting) { resetAllMeasurements(measure.measurementData); return; }
        ...
        if (phNo?.trim()) {
          try {
            const { data, error, status } = await supabase
              ...
              .select(`id, dress_type, measurement_data, customer_id, Customer!inner(id, custName, phoneNo)`)
            ...
            if (error && status !== 406) {
              ...
              if (resetExisting) { resetAllMeasurements(measure.measurement_data); return; }
              populateMeasurements(data);
              if (data) {
                const cacheData = { dressType: routeParams?.itemName, measurementData: data.measurement_data };
  ...
  const populateMeasurements = (res) => {
    if (res === null || res.length === 0 || routeParams?.itemName === 'Alteration') {
      ...
      const newItem = {
        id: uniqId, localId: newCo, dressType: routeParams?.itemName,
        dressSubType: optionsSub[0], dueDate: moment(new Date()).format("YYYY-MM-DD"),
        stitchingAmt: '', frontNeckType: '', backNeckType: '', sleeveType: '', sleeveLength: '',
        measurementData: completeMeasurementData, notes: '', frontNeckDesignFile: null,
        backNeckDesignFile: null, sleeveDesignFile: null,
        dressPics: [], patternPics: [], measurementPics: [],
        dressGiven: false, alterDressType: "", extraMeasurements: {},
        editable: false, repeatDesign: true, repeatMeas: true, extraOptions: {},
        slotDates: [], slots: {}, savedDesign: false, savedMeas: false
      };
  ...
  const handleNextStep = async () => {
    ...
    if (custName.trim() === '') { setCustNameError(true); }
    else if (phoneNo.trim() === '') { setPhoneError(true); }
    ...
    <View>
      {localItems.map((item, index) => (
        <RenderItemCollapsible
          ...
          updateItemMultiple={updateItemMultiple}
          saveAllLocalStates={saveAllLocalStates}
          deleteItemAlert={deleteItemAlert}
          navigation={navigation}
          optionsSub={optionsSub}
            ...
            {step === 1 ? (
                ...
                <View>
                  ...
                  <Card style={styles.card} status="primary">
                    ...
                    <Input
                      ...
                      label='Customer name *'
                      style={styles.formInput}
                      accessoryRight={PersonIconLocal}
                      value={custName}
                      onChangeText={(text) => {
                        setCustName(text);
              ...
              <>
                ...
                <Layout style={styles.footer}>
                  <Button appearance='outline' onPress={handleIncrement}>Add more</Button>
                  <Button onPress={handleSave}>Confirm order</Button>
                </Layout>
              </>
            )}
          </Layout>
...
const styles = StyleSheet.create({
  ...
  header: { position: 'relative', height: 220, marginBottom: -45 },
  headerImage: { width: '100%', height: '80%', borderBottomLeftRadius: 25, borderBottomRightRadius: 25, resizeMode: 'cover' },
  avatar: { position: 'absolute', right: 20, bottom: -30, width: 60, height: 60, borderWidth: 3, borderColor: '#FFFFFF' },
  changeButton: { position: 'absolute', right: 20, bottom: -55 },
  content: { padding: 20, overflow: 'visible' },
  title: { marginBottom: 10 },
  shopCard: { marginTop: 20, padding: 15, borderRadius: 10, backgroundColor: '#F7F9FC', flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' },
  addressCard: { marginTop: 20, padding: 15, borderRadius: 10, backgroundColor: '#F7F9FC' },
  icon: { width: 24, height: 24, marginRight: 10 },
  footer: { flexDirection: 'row', marginTop: 10, justifyContent: 'space-between', alignItems: 'center', marginHorizontal: 40 },
  bookNowButton: { flex: 1 },
  quantityContainer: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginTop: 10, paddingHorizontal: 10, backgroundColor: '#F7F9FC', height: 50 },
  selectContainer: { width: 310, marginTop: -5 },
  priceContainer: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 10, backgroundColor: '#F7F9FC', height: 50, width: 310, marginTop: 10 },
  alterContainer: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 10, backgroundColor: '#F7F9FC', height: 50, width: 310, marginTop: 5 },
  priceContainer1: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 10, backgroundColor: '#F7F9FC', height: 50, width: 310, marginTop: 10, marginBottom: -5 },
  dateContainer: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginTop: 10, paddingHorizontal: 10, backgroundColor: '#F7F9FC', height: 50, width: 310, position: 'relative' },
  ...
  formInput: { marginTop: 16 },
  nextButton: { marginTop: 25, width: 100 },
  cancelButton: { marginTop: 25, width: 150 },
  itemContainer: { borderRadius: 8, width: 310 },
  buttonContainer: { flexDirection: 'row', justifyContent: 'space-between', marginTop: 16 },
  label: { marginLeft: 10 },
  uploadButton: { marginTop: 7, borderRadius: 8, alignItems: 'center', justifyContent: 'center', width: 110, height: 100, padding: 0, backgroundColor: '#F7F9FC' },
  uploadButton1: { marginTop: 7, borderRadius: 8, alignItems: 'center', justifyContent: 'center', width: 100, height: 100, padding: 0, backgroundColor: '#F7F9FC' },
  uploadContent: { alignItems: 'center', justifyContent: 'center' },
  uploadIcon: { width: 32, height: 32, marginBottom: 4, marginLeft: 15 },
  uploadIcon1: { width: 32, height: 32, marginBottom: 4, marginLeft: 5 },
  uploadButtonText: { textAlign: 'center', fontSize: 12 },
  imageContainer: { flexDirection: 'row', flexWrap: 'wrap', marginTop: 16, marginLeft: -10, marginHorizontal: 15 },
  imageWrapper: { position: 'relative', margin: 8 },
  image: { width: 100, height: 100, borderRadius: 8 },
  deleteIcon: { width: 24, height: 24 },
  modalOption: { padding: 15, alignItems: 'center', backgroundColor: '#fff', borderRadius: 5, margin: 5 },
  modalIcon: { marginRight: 10 },
  modalOptionRow: { flexDirection: 'row', justifyContent: 'space-between', marginBottom: -20 },
  tabContent: { padding: 16, paddingLeft: 30, borderRadius: 8, width: 320 },
  tabContentRow: { flexDirection: 'row', gap: 30, flexWrap: 'wrap' },
  fieldLabel: { marginLeft: -20 },
  fieldContainer1: { margin: 16, marginTop: 25, marginLeft: 25 },
  fieldContainer3: { marginTop: 10, marginLeft: 25 },
  fieldContainer2: { flexDirection: 'row', justifyContent: 'space-between', marginBottom: 16, marginLeft: -10, alignItems: 'center' },
  labelText1: { width: 180, marginRight: 8, marginTop: 5, flexWrap: 'wrap' },
  fieldContainerDG: { flexDirection: 'row', justifyContent: 'space-between', marginBottom: 16, marginLeft: -10, marginTop: 10, alignItems: 'center' },
  imageDesign: { width: 100, height: 100, marginHorizontal: 8, marginBottom: 15 },
  deleteButton: { borderRadius: 10, marginTop: 5, zIndex: 1, width: 100 },
  editButton: { marginLeft: 25, width: 50, marginBottom: 10, marginTop: 10 },
  editButton1: { width: 100, marginBottom: 10, marginTop: 10 },
  drawButton: { borderRadius: 10, marginBottom: 15, marginTop: -5, zIndex: 1, width: 200, alignItems: 'center' },
  overlay: { position: 'absolute', padding: 5, top: 0, left: 0, right: 0, bottom: 0, justifyContent: 'center', alignItems: 'center', backgroundColor: 'rgba(0, 0, 0, 0.5)', borderRadius: 10 },
  overlayText: { color: '#fff', textAlign: 'center', marginTop: 40 },
  carouselContainer: { position: 'relative' },
  deleteButtonImg: { position: 'absolute', top: 10, right: 10 },
  carouselImage: { width: 100, height: 100 },
  deleteImgButton: { position: 'absolute', bottom: -50, right: 10 },
  fullScreenModal: { justifyContent: 'center', alignItems: 'center', width: '100%', height: '100%' },
  fullScreenModalMeas: { justifyContent: 'center', alignItems: 'center', height: 600, borderRadius: 20 },
  backdrop: { backgroundColor: 'rgba(0, 0, 0, 0.5)' },
  closeButton: { position: 'absolute', top: -10, right: -10, backgroundColor: 'rgba(255, 255, 255, 0.8)', borderRadius: 20, padding: 2 },
  closeButtonDesign: { position: 'absolute', top: -10, right: -10, backgroundColor: 'rgba(255, 255, 255, 0.8)', borderRadius: 20, padding: 2 },
  closeIcon: { width: 20, height: 20 },
  closeButtonOuter: { position: 'absolute', top: 30, right: -10, backgroundColor: 'rgba(255, 255, 255, 0.8)', borderRadius: 20, padding: 2 },
  modalContent: { padding: 20, marginLeft: -20, position: 'relative' },
  modalCloseButton: { position: 'absolute', top: -30, right: -30, backgroundColor: 'rgba(255, 255, 255, 0.8)', borderRadius: 15, padding: 5 },
  modalCloseIcon: { width: 30, height: 30 },
  spinner: { flex: 1, justifyContent: 'center', alignItems: 'center', color: 'color-primary-500' },
  selectField: { width: 150, marginRight: -30 },
  selectFieldRadio: { flexDirection: 'row', marginLeft: 5, marginTop: 5, flexWrap: 'wrap', marginLeft: -20 },
  selectFieldOcc: { marginTop: 10 },
  switchContainer: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginTop: 10, padding: 10 },
  subView: { position: 'absolute', bottom: 0, left: 0, right: 0, backgroundColor: '#FFFFFF', zIndex: 10, elevation: 5 },
  subView1: { position: 'absolute', bottom: 0, left: 0, right: 0, backgroundColor: '#FFFFFF', zIndex: 10, elevation: 5 },
  innerLayout: { flexDirection: 'row', alignItems: 'center', marginHorizontal: 5, backgroundColor: '#F7F9FC' },
  buttonTextContainer: { flexDirection: "row", alignItems: "center", marginLeft: -35, marginTop: -10 },
  inputsContainer: { flexDirection: 'row', justifyContent: 'space-between', marginBottom: 12 },
  card: { borderRadius: 10, backgroundColor: '#F7F9FC', elevation: 5 },
  itemsScrollView: { marginTop: 10, marginBottom: 10 },
  collapsibleItemContainer: { marginBottom: 10, borderRadius: 8, overflow: 'hidden', borderWidth: 1, borderColor: '#E4E9F2', backgroundColor: 'white' },
  collapsibleHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', padding: 16, backgroundColor: '#F7F9FC' },
  itemTitle: { fontWeight: 'bold' },
  expandIcon: { width: 24, height: 24 },
  trashIcon: { marginLeft: -10, marginRight: -10 },
  collapsibleContent: { padding: 16, backgroundColor: 'white' },
  datePicker: { position: 'relative' },
  checkboxText: { marginLeft: 10 },
  checkboxStyle: { fontWeight: 'bold', fontSize: 14 },
  ...
  focusedInput: { borderColor: '#000099', backgroundColor: 'white' },
  errorInput: { borderColor: 'red', backgroundColor: 'white' },
  ...
  selectedContent: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', width: '100%' },
  editIcon: { width: 16, height: 16, marginLeft: 8 },
  designButton: { marginHorizontal: 90, marginTop: 10 },
  designButtonText: { textAlign: 'center', fontSize: 12, fontWeight: 'bold' },
  labelText: { marginBottom: 5 },
  nextButtonContainer: { flexDirection: 'row', gap: 30, justifyContent: 'center' },
  navButton: { marginLeft: 20 },
  scrollView: { width: '105%', height: 400, marginLeft: -10 },
  scrollViewContent: { paddingVertical: 10, paddingHorizontal: 10 },
  tooltipContent: { padding: 10 },
  tooltipTitle: { fontSize: 18, fontWeight: 'bold', color: '#333', marginBottom: 10 },
  tooltipText: { fontSize: 15, color: '#666', lineHeight: 22, marginBottom: 10, textAlign: 'justify' },
  tooltipButtons: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' },
  skipText: { color: '#999', fontSize: 15 },
  nextButton1: { backgroundColor: '#007AFF', paddingHorizontal: 20, paddingVertical: 10, borderRadius: 8 },
  nextText: { color: 'white', fontSize: 15, fontWeight: '600' },
  imageContainerMeas: { position: 'relative', marginRight: 12, marginTop: 10 },
  previewImage: { width: 80, height: 80, borderRadius: 8 },
  imagePreview: { maxHeight: 100 },
  addOnItem: { marginBottom: 8, backgroundColor: 'transparent' },
  addOnInputContainer: { flexDirection: 'row', alignItems: 'center' },
  addOnNameInput: { marginRight: 8, paddingHorizontal: 12, paddingVertical: 8 },
  addOnPriceInput: { marginRight: 8, paddingHorizontal: 12, paddingVertical: 8 },
  removeButton: { paddingHorizontal: 8 },
  removeIcon: { width: 16, height: 16 },
  accordionCard: { borderRadius: 8, marginTop: 5 },
  headerContent: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' },
  headerLeft: { flexDirection: 'row', alignItems: 'center', backgroundColor: 'transparent' },
  headerIcon: { width: 20, height: 20, marginRight: 15, marginLeft: -7 },
  headerText: { fontWeight: '600' },
  headerRight: { flexDirection: 'row', alignItems: 'center', backgroundColor: 'transparent' },
  priceText: { fontSize: 20, fontWeight: 'bold', marginRight: 12 },
  chevronIcon: { width: 20, height: 20 },
  expandedContent: { marginTop: 10, borderTopWidth: 1, borderTopColor: '#e4e4e7', backgroundColor: 'transparent' },
  stitchingContainer: { padding: 16, backgroundColor: 'transparent', flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' },
  inputHeader: { flexDirection: 'row', alignItems: 'center', marginBottom: 12, backgroundColor: 'transparent' },
  inputIcon: { width: 16, height: 16, marginRight: 8 },
  inputLabel: { marginTop: 5 },
  addonsAccordion: { backgroundColor: '#f8f9fa' },
  addonsHeader: { padding: 12 },
  addonsHeaderContent: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', backgroundColor: 'transparent' },
  addonsHeaderLeft: { flexDirection: 'row', alignItems: 'center', backgroundColor: 'transparent' },
  addonsIcon: { width: 16, height: 16, marginRight: 8 },
  addonsHeaderText: { fontWeight: '500' },
  addonsChevron: { width: 16, height: 16 },
  addonsContent: { backgroundColor: '#ffffff', borderTopWidth: 1, borderTopColor: '#e4e4e7' },
  addonItem: { paddingHorizontal: 16, paddingVertical: 12, borderBottomWidth: 1, borderBottomColor: '#f1f5f9', backgroundColor: 'transparent' },
  addonRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', backgroundColor: 'transparent' },
  addonName: { flex: 1, marginRight: 12 },
  sectionContent: { padding: 16 },
  tableContainer: { backgroundColor: '#fff' },
  tableHeaderRow: { flexDirection: 'row', backgroundColor: '#dbeafe', padding: 8 },
  tableHeaderCell: { flex: 1, fontSize: 14, fontWeight: '600', color: '#374151', paddingHorizontal: 8 },
  centerText: { textAlign: 'center' },
  tableDataRow: { flexDirection: 'row', borderBottomWidth: 1, borderBottomColor: '#e5e7eb', padding: 8 },
  tableDataCell: { flex: 1, fontSize: 14, color: '#374151', paddingHorizontal: 8, fontWeight: '500' },
  centerAlign: { alignItems: 'center', justifyContent: 'center' },
  slotBadge: { backgroundColor: '#dbeafe', paddingHorizontal: 8, paddingVertical: 2, borderRadius: 4 },
  expressSlotBadge: { backgroundColor: '#fed7aa', paddingHorizontal: 8, paddingVertical: 2, borderRadius: 4 },
  slotBadgeText: { fontSize: 12, fontWeight: '600', color: '#1e40af' },
  celebrationCard: { marginTop: 20, marginBottom: -10, backgroundColor: '#F7F9FC', borderRadius: 10, elevation: 5 },
  celebrationItem: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', backgroundColor: 'white', padding: 12, borderRadius: 8, marginBottom: 8, borderWidth: 1, borderColor: '#e5e7eb' },
  celebrationLeft: { flex: 1 },
  celebrationType: { fontWeight: 'bold', color: '#374151' },
  celebrationDate: { color: '#6b7280', marginTop: 2 },
  celebrationVal: { color: '#6b7280', marginTop: 2, marginLeft: 22 },
  daysBadge: { paddingHorizontal: 12, paddingVertical: 6, borderRadius: 16 },
  todayBadge: { backgroundColor: '#fee2e2' },
  soonBadge: { backgroundColor: '#fed7aa' },
  upcomingBadge: { backgroundColor: '#dbeafe' },
  daysText: { fontSize: 12, fontWeight: 'bold' },
  todayText: { color: '#dc2626' },
  soonText: { color: '#ea580c' },
  upcomingText: { color: '#2563eb' },
  foundCard: { marginBottom: 16, backgroundColor: '#f0fdf4' },
  newCustomerCard: { marginTop: 20, marginBottom: -10, borderRadius: 10, backgroundColor: '#F7F9FC', elevation: 5 },
  newCustomerTitle: { textAlign: 'center' },
  newCustomerSubtitle: { textAlign: 'center', marginLeft: -20, marginBottom: 10 },
  celebrationRow: { flexDirection: 'row', justifyContent: 'space-between', marginBottom: 8 },
  celebrationItemHalf: { flex: 0.48 },
  custSaveButton: { marginTop: 10, marginHorizontal: 100 },
  ...
  cameraControls: { position: 'absolute', bottom: 40, left: 0, right: 0, flexDirection: 'row', justifyContent: 'space-evenly', alignItems: 'center', paddingHorizontal: 20 },
  captureBtn: { flex: 1, marginRight: 10 },
  cancelBtn: { flex: 1, marginLeft: 10 },
  sectionContainer: {},
  accordionHeader: {},
  collapseHeader: {},
});
